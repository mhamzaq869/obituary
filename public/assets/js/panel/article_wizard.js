    ...
    if (stream_type == 'backend') {
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                messages: [{
                    role: 'user',
                    content: `Generate ${keywords_count} keywords(simple words or 2 words, not phrase, not person name) about '${keywords_topic}'. in ${language} language. Must resut as array json data. Result format is [keyword1, keyword2, ..., keywordn].`
                }],
                model: openai_model
            }),
            success: function (data) {
        ...
        let asyncGenerate = async () => {
            const response = await fetch(atob(guest_id), {
                ...
                body: JSON.stringify({
                    ...
                    messages: [{
                        role: 'user',
                        content: `Write Article(Maximum  ${length} words). in ${language} language. Generate article (Must not contain title, Must Mark outline with <h3> tag) about $title with following outline${JSON.parse(wizardData.outline).join(",")}. Must mark outline with <h3> tag. `,
                    }],
